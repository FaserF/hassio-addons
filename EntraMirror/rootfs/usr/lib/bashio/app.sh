...
bashio::app.auto_migrate_data() {
	...
	if [ -d "/share" ]; then
		...
		if [ -d "$share_backup" ] && [ -z "$(ls -A /data 2>/dev/null | grep -E '\.db|\.sqlite|database|\.secret_key')" ]; then
		...
		if [ -d "/data" ]; then
			...
			if [ -d "/data/database" ]; then
				mkdir -p "$share_backup/database"
				cp -ru /data/database/* "$share_backup/database/" 2>/dev/null || true
			fi
		fi
	fi
}
