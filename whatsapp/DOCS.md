...
**Flow:**
`Home Assistant` -> `WhatsApp Integration` -> `HTTP (Port 8000)` -> `This Addon` -> `Playwright (Chrome)` -> `WhatsApp Web`
...
## Persistence
Your session data is stored in `/data`. This directory is persistent across add-on updates and restarts. ensuring you don't have to scan the QR code frequently.
