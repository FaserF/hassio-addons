...
app.get('*', (req, res) => {
  if (req.path.startsWith('/api') || req.path === '/qr' || req.path === '/status' || req.path === '/session/start' || req.path === '/send_message' || req.path === '/session' || req.path === '/logs') {
    return res.status(404).send('Not Found')
  }

  // Determine current state
  const statusClass = isConnected ? 'connected' : (currentQR ? 'waiting' : 'disconnected')
  const statusText = isConnected ? 'Connected 🟢' : (currentQR ? 'Scan QR Code 📱' : 'Disconnected 🔴')
  const showQR = !isConnected && currentQR
  const showQRPlaceholder = !isConnected && !currentQR

  // Recent logs (last 10)
  const recentLogs = connectionLogs.slice(0, 10).map(l =>
