...
export function registerAPIRoutes(app) {
  ...
  app.post('/session/start', (req, res) => {
    ...
    if (session.sock && !session.sock.ws?.isClosed) return res.json({ status: 'scanning', message: 'Session negotiation in progress' });
  ...
  app.delete('/session', async (req, res) => {
    ...
    try {
      ...
      session.stats = {
        ...
        last_sent_message: 'None', last_sent_target: 'None',
        last_received_message: 'None', last_received_sender: 'None',
        last_failed_message: 'None', last_failed_target: 'None',
        last_error_reason: 'None',
        last_sent_time: null, last_received_time: null, last_failed_time: null,
        start_time: Date.now(), my_number: 'Unknown',
  ...
  app.get('/status', (req, res) => {
    const session = getReqSession(req);
    res.json({ connected: session.isConnected, version: BAILEYS_VERSION, session_id: session.id, disconnect_reason: session.isConnected ? null : session.disconnectReason });
  });
  ...
  app.get('/stats', (req, res) => {
    const session = getReqSession(req);
    res.json({ ...session.stats, uptime: Math.floor((Date.now() - session.stats.start_time) / 1000) });
  });
  ...
  app.post('/send_image', async (req, res) => {
    ...
    try {
      const jid = getJid(number);
      const sentMsg = await session.sock.sendMessage(jid, { image: { url: url }, caption: caption }, { quoted, ephemeralExpiration: expiration });
      session.stats.sent += 1;
  ...
  app.post('/send_poll', async (req, res) => {
    ...
    try {
      const jid = getJid(number);
      const sentMsg = await session.sock.sendMessage(jid, { poll: { name: question, values: options, selectableCount: 1 } }, { quoted, ephemeralExpiration: expiration });
      session.stats.sent += 1;
  ...
  app.post('/send_location', async (req, res) => {
    ...
    try {
      const jid = getJid(number);
      const sentMsg = await session.sock.sendMessage(jid, { location: { degreesLatitude: latitude, degreesLongitude: longitude, name: title, address: description } }, { quoted, ephemeralExpiration: expiration });
      session.stats.sent += 1;
  ...
  app.post('/send_reaction', async (req, res) => {
    ...
    try {
      const jid = getJid(number);
      const sentMsg = await session.sock.sendMessage(jid, { react: { text: reaction, key: { remoteJid: jid, fromMe: false, id: messageId } } });
      res.json({ status: 'sent', id: sentMsg.key.id });
    } catch (e) {
      session.stats.failed += 1;
      res.status(500).json({ detail: e.toString() });
  ...
  app.post('/send_buttons', async (req, res) => {
    ...
    try {
      ...
      const formattedButtons = (buttons || []).map((b) => ({
        name: 'quick_reply',
        buttonParamsJson: JSON.stringify({ display_text: b.displayText || b.text || 'Button', id: b.id || b.buttonId || String(Math.random()) }),
      }));
      const messageId = generateMessageID();
      await session.sock.relayMessage(jid, { viewOnceMessage: { message: { interactiveMessage: { header: { title: '', hasMediaAttachment: false }, body: { text: message }, footer: { text: footer || '' }, nativeFlowMessage: { buttons: formattedButtons } } } } }, { messageId, quoted, ephemeralExpiration: expiration });
      session.stats.sent += 1;
  ...
  app.post('/send_document', async (req, res) => {
    ...
    try {
      const jid = getJid(number);
      const sentMsg = await session.sock.sendMessage(jid, { document: { url: url }, fileName: fileName, caption: caption, mimetype: 'application/octet-stream' }, { quoted, ephemeralExpiration: expiration });
      session.stats.sent += 1;
  ...
  app.post('/send_video', async (req, res) => {
    ...
    try {
      const jid = getJid(number);
      const sentMsg = await session.sock.sendMessage(jid, { video: { url: url }, caption: caption }, { quoted, ephemeralExpiration: expiration });
      session.stats.sent += 1;
      session.stats.last_sent_time = Date.now();
  ...
  app.post('/send_audio', async (req, res) => {
    ...
    try {
      const jid = getJid(number);
      await session.sock.sendMessage(jid, { audio: { url: url }, ptt: !!ptt, mimetype: 'audio/mp4' }, { quoted, ephemeralExpiration: expiration });
      session.stats.sent += 1;
  ...
  app.post('/revoke_message', async (req, res) => {
    ...
    try {
      const jid = getJid(number);
      const key = { remoteJid: jid, fromMe: fromMe !== undefined ? Boolean(fromMe) : true, id: message_id };
  ...
  app.get('/groups', async (req, res) => {
    ...
    try {
      ...
      const result = Object.values(groups).map((g) => ({ id: g.id, name: g.subject, participants: g.participants.length }));
      res.json(result);
    } catch (e) {
      res.status(500).json({ detail: e.toString() });
    }
  ...
  app.post('/send_list', async (req, res) => {
    ...
    try {
      ...
      const formattedSections = (sections || []).map((s) => ({
        title: s.title || '',
        rows: (s.rows || []).map((r) => ({ header: r.title || '', title: r.title || '', description: r.description || '', id: r.id || String(Math.random()) })),
      }));
      const messageId = generateMessageID();
      await session.sock.relayMessage(jid, { viewOnceMessage: { message: { interactiveMessage: { header: { title: title || '', hasMediaAttachment: false }, body: { text: text || 'Menu' }, footer: { text: footer || '' }, nativeFlowMessage: { buttons: [{ name: 'single_select', buttonParamsJson: JSON.stringify({ title: button_text || 'Open Menu', sections: formattedSections }) }] } } } } }, { messageId, quoted, ephemeralExpiration: expiration });
      session.stats.sent += 1;
  ...
  app.post('/send_contact', async (req, res) => {
    ...
    try {
      ...
      const quoted = getQuotedMessage(session, quotedMessageId);
      await session.sock.sendMessage(jid, { contacts: { displayName: contact_name, contacts: [{ vcard }] } }, { quoted, ephemeralExpiration: expiration });
      session.stats.sent += 1;
  ...
  app.get('/health', (req, res) => res.status(200).json({ status: 'ok', service: 'whatsapp-homeassistant-app' }));
  ...
  app.get('/api/dashboard', (req, res) => {
    ...
    const sessionList = Array.from(sessions.keys()).map(sid => ({ id: sid, connected: sessions.get(sid).isConnected, number: sessions.get(sid).stats?.my_number || 'Unknown' }));
    res.json({
      sessionId: session.id, isConnected: session.isConnected, currentQR: session.currentQR, disconnectReason: session.disconnectReason, reconnectAttempts: session.reconnectAttempts,
      stats: session.stats || { sent: 0, received: 0, failed: 0 },
      uptime: session.stats?.start_time ? new Date(Date.now() - session.stats.start_time).toISOString().substr(11, 8) : 'N/A',
      sessionList, recentLogs: (session.connectionLogs || []).slice(0, 10), recentSent: (session.recentSent || []).slice(0, 5), recentReceived: (session.recentReceived || []).slice(0, 5), recentFailures: (session.recentFailures || []).slice(0, 5),
      nodeVersion: process.version, addonVersion: ADDON_VERSION, integrationVersion: INTEGRATION_VERSION, baileysVersion: BAILEYS_VERSION,
      webhookEnabled: WEBHOOK_ENABLED, webhookUrl: WEBHOOK_URL, deviceInfo: session.deviceInfo || {},
    });
  });
