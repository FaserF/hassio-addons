...
export function registerTelegramRoutes(app) {
  ...
  app.post('/api/telegram/mappings', (req, res) => {
    ...
    const cleanTgTitle = (tg_chat_title && !tg_chat_title.startsWith('Chat ')) ? tg_chat_title : `TG ${tg_chat_id}`;
    const threadLabel = tg_thread_id ? ` (Topic ${tg_thread_id})` : '';
    const autoGeneratedName = `${cleanWaName} ↔ ${cleanTgTitle}${threadLabel}`;
    const finalMappingName = (mapping_name && String(mapping_name).trim()) ? String(mapping_name).trim() : autoGeneratedName;
