...
registry.register('setrules', async (session, groupId, userId, args, config) => {
  const text = args.join(' ');
  if (!text) {
    await reply(session, groupId, { text: `⚠️ Usage: \`${config.commands.prefix}setrules <text>\`` });
    return;
  }
...
registry.register('promote', async (session, groupId, userId, args, config, isAdminUser, rawMsg) => {
  ...
  if (targetMatches.length === 0) {
    await reply(session, groupId, { text: `⚠️ You must mention a user or reply to their message to promote them.` });
    return;
  }
  try {
    await session.sock.groupParticipantsUpdate(groupId, targetMatches, 'promote');
    await reply(session, groupId, { text: `✅ Promoted ${targetMatches.length} user(s) to Admin.`, mentions: targetMatches });
  } catch (e) {
    await reply(session, groupId, { text: `❌ Failed to promote users.` });
  }
}, { adminOnly: true, help: 'Promote a user to Admin' });

registry.register('demote', async (session, groupId, userId, args, config, isAdminUser, rawMsg) => {
  const targetMatches = [...(rawMsg.message?.extendedTextMessage?.contextInfo?.mentionedJid || [])];
  if (targetMatches.length === 0 && rawMsg.message?.extendedTextMessage?.contextInfo?.participant) {
    targetMatches.push(rawMsg.message.extendedTextMessage.contextInfo.participant);
  }
  if (targetMatches.length === 0) {
    await reply(session, groupId, { text: `⚠️ You must mention a user or reply to their message to demote them.` });
    return;
  }
  try {
    await session.sock.groupParticipantsUpdate(groupId, targetMatches, 'demote');
    await reply(session, groupId, { text: `✅ Demoted ${targetMatches.length} user(s) from Admin.`, mentions: targetMatches });
  } catch (e) {
    await reply(session, groupId, { text: `❌ Failed to demote users.` });
  }
}, { adminOnly: true, help: 'Demote an Admin to regular user' });

registry.register('report', async (session, groupId, userId, args, config, isAdminUser, rawMsg) => {
...
registry.register('approve', async (session, groupId, userId, args, config, isAdminUser, rawMsg) => {
  const targetMatches = [...(rawMsg.message?.extendedTextMessage?.contextInfo?.mentionedJid || [])];
  if (targetMatches.length === 0 && rawMsg.message?.extendedTextMessage?.contextInfo?.participant) {
    targetMatches.push(rawMsg.message.extendedTextMessage.contextInfo.participant);
  }
  ...
  saveModerationStore(store);
  await reply(session, groupId, { text: `✅ Approved ${targetMatches.length} user(s). They will bypass moderation locks.`, mentions: targetMatches });
}, { adminOnly: true, help: 'Whitelist a user from moderation locks' });

registry.register('unapprove', async (session, groupId, userId, args, config, isAdminUser, rawMsg) => {
  const targetMatches = [...(rawMsg.message?.extendedTextMessage?.contextInfo?.mentionedJid || [])];
  if (targetMatches.length === 0 && rawMsg.message?.extendedTextMessage?.contextInfo?.participant) {
    targetMatches.push(rawMsg.message.extendedTextMessage.contextInfo.participant);
  }
  ...
  saveModerationStore(store);
  await reply(session, groupId, { text: `✅ Removed approval for ${targetMatches.length} user(s).`, mentions: targetMatches });
}, { adminOnly: true, help: 'Remove user from whitelist' });

registry.register('setwelcome', async (session, groupId, userId, args, config) => {
  const text = args.join(' ');
  if (!text) {
    await reply(session, groupId, { text: `⚠️ Usage: \`${config.commands.prefix}setwelcome <text>\`\nPlaceholders: {mention}, {name}, {group}` });
    return;
  }
  const store = loadModerationStore();
  const c = store.groups[groupId] || getGroupModerationConfig(groupId);
...
registry.register('welcome', async (session, groupId, userId, args, config) => {
...
registry.register('setgoodbye', async (session, groupId, userId, args, config) => {
  const text = args.join(' ');
  if (!text) {
    await reply(session, groupId, { text: `⚠️ Usage: \`${config.commands.prefix}setgoodbye <text>\`\nPlaceholders: {mention}, {name}, {group}` });
    return;
  }
  const store = loadModerationStore();
  const c = store.groups[groupId] || getGroupModerationConfig(groupId);
...
registry.register('goodbye', async (session, groupId, userId, args, config) => {
...
registry.register('save', async (session, groupId, userId, args, config) => {
  if (args.length < 2) {
    await reply(session, groupId, { text: `⚠️ Usage: \`${config.commands.prefix}save <notename> <content>\`` });
    return;
  }
  ...
  saveModerationStore(store);
  await reply(session, groupId, { text: `✅ Saved note \`${name}\`\nRetrieve it with \`${config.commands.prefix}get ${name}\` or \`#${name}\`` });
}, { adminOnly: true, help: 'Save a text note' });

registry.register('get', async (session, groupId, userId, args, config) => {
  if (args.length < 1) {
    await reply(session, groupId, { text: `⚠️ Usage: \`${config.commands.prefix}get <notename>\`` });
    return;
  }
  const name = args[0].toLowerCase();
...
registry.register('notes', async (session, groupId, userId, args, config) => {
  if (!config.notes || Object.keys(config.notes).length === 0) {
    await reply(session, groupId, { text: `📝 No notes saved.` });
    return;
  }
  const list = Object.keys(config.notes).map(n => `• #${n}`).join('\n');
  await reply(session, groupId, { text: `📝 *Saved Notes:*\n${list}` });
}, { adminOnly: false, help: 'List all saved notes' });

registry.register('filter', async (session, groupId, userId, args, config) => {
  if (args.length < 2) {
    await reply(session, groupId, { text: `⚠️ Usage: \`${config.commands.prefix}filter <trigger> <reply>\`` });
    return;
  }
...
registry.register('stop', async (session, groupId, userId, args, config) => {
  if (args.length < 1) {
    await reply(session, groupId, { text: `⚠️ Usage: \`${config.commands.prefix}stop <trigger>\`` });
    return;
  }
  const trigger = args[0].toLowerCase();
...
registry.register('filters', async (session, groupId, userId, args, config) => {
  if (!config.filters || config.filters.length === 0) {
    await reply(session, groupId, { text: `No filters configured.` });
    return;
  }
  const list = config.filters.map(f => `• \`${f.trigger}\` -> ${f.is_regex ? '(Regex)' : ''}`).join('\n');
