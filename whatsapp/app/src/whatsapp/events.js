...
import { notifyAdmins, trackReceived, trackFailure, handleFirstContact, reply, runDiagnostic } from './actions.js';
...
export async function checkSystemUpdates(session) {
  ...
  if (SYSTEM_STATE.last_addon_version !== 'Unknown' && SYSTEM_STATE.last_addon_version !== currentAddonVersion) {
    updateMessages.push(`📦 *WhatsApp App Updated*\n• *Version:* ${SYSTEM_STATE.last_addon_version} ➔ ${currentAddonVersion}`);
  }

  if (SYSTEM_STATE.last_integration_version !== 'Unknown' && SYSTEM_STATE.last_integration_version !== currentIntegrationVersion) {
    updateMessages.push(`🧩 *Integration Updated*\n• *Version:* ${SYSTEM_STATE.last_integration_version} ➔ ${currentIntegrationVersion}`);
  }
  ...
  if (updateMessages.length > 0) {
    const fullText = `🔔 *System Status Update*\n• *Time:* ${now}\n\n` + updateMessages.join('\n\n');
...
export async function monitorHACore(session) {
  setInterval(async () => {
    ...
    if (!isOnline && !SYSTEM_STATE.last_ha_online) {
      ...
      notifyAdmins(session, `🔴 *Home Assistant Core Unreachable*\n\n• *Status:* Bot can no longer reach HA Core.\n• *Note:* Automations are temporarily offline.`).catch(() => {});
...
export function handleIncomingMessages(session) {
    session.sock.ev.on('messages.upsert', async (m) => {
        ...
        const events = m.messages
            ...
            .map(async (msg) => {
                let text = msg.message?.conversation || msg.message?.extendedTextMessage?.text || msg.message?.buttonsResponseMessage?.selectedDisplayText || msg.message?.templateButtonReplyMessage?.selectedId || '';
                ...
                let mediaUrl = null, mediaPath = null, mediaType = null, mimeType = null, caption = null;
                ...
                if (supportedMediaTypes.includes(messageType)) {
                    try {
                        ...
                        const buffer = await downloadMediaMessage(msg, 'buffer', {}, { logger: logger.child({ module: `media-dl-${session.id}` }) });
                        if (buffer) {
                    ...
                    if (body.startsWith('ha-app-')) {
                        ...
                        if (body === 'ha-app-ping') {
                            ...
                            if (recentRequests.length >= 3) {
                                logger.warn({ personJid: maskData(personJid), sessionId: session.id }, 'Rate limit hit for status command');
                                return await reply(session, senderJid, { text: '⏳ *Rate Limit:* Please wait a minute before requesting status again.' });
                            }
                            recentRequests.push(now);
                            session.statusRateLimit.set(personJid, recentRequests);

                            const haInfo = await fetchHAVersions();
                            const statusText = `🤖 *WhatsApp Bridge Status*\n\n` +
                            ...
                            await reply(session, senderJid, { text: '🔄 *Restarting...*\nThe connection will be reset in 2 seconds.' });
                            addLog(session, `Admin ${maskData(personJid)} requested restart`, 'warning');
                            setTimeout(() => { session.sock.end(new Error('Admin requested restart')); }, 2000);
                            ...
                            if (logs.length === 0) {
                                ...
                                const logText = logs.map((l) => `[${l.timestamp}] ${l.msg}`).reverse().join('\n');
                                await reply(session, senderJid, { text: `📜 *Recent Connection Events:*\n\n${logText}` });
                            }
                        } else if (body.startsWith('ha-app-stats')) {
                            const range = body.replace('ha-app-stats', '').trim() || 'all-time';
                            const statsText = `📈 *Message Statistics (${range})*\n\n` +
                            ...
                            const helpText = `📖 *WhatsApp Bridge Help*\n\n` +
                                `*General Commands:*\n` +
