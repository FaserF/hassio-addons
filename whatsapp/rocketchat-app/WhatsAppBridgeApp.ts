...
export class WhatsAppBridgeApp extends App implements IPostMessageSent {
  ...
  public async checkPostMessageSent?(
    ...
    const targetRoomName = await read
      .getEnvironmentReader()
      .getSettings()
      .getValueById(AppSetting.TargetRoomName);
    return (
  ...
  public async executePostMessageSent(
    ...
    const url = await read
      .getEnvironmentReader()
      .getSettings()
      .getValueById(AppSetting.WhatsAppAddonUrl);
    const token = await read
      .getEnvironmentReader()
      .getSettings()
      .getValueById(AppSetting.WhatsAppApiToken);
    ...
    if (!targetNumber) {
      this.getLogger().debug(
        'No target number found in message prefix [number]. Skipping WhatsApp forward.'
      );
      return;
    }
