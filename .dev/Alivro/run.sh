...
# --- CONFIGURATION ---
DATA_DIR="/data/alivro"
DB_DIR="$DATA_DIR/database"
PLUGINS_DIR="$DATA_DIR/plugins"
UPLOADS_DIR="$DATA_DIR/uploads"

bashio::log.info "Starting Alivro App initialization..."
...
if bashio::config.true 'reset_database'; then
	...
	BACKUP_DIR="/data/alivro_backups"
	mkdir -p "$BACKUP_DIR"
	TIMESTAMP=$(date +%Y%m%d_%H%M%S)
	BACKUP_FILE="$BACKUP_DIR/alivro_backup_${TIMESTAMP}.tar.gz"
	...
	rm -f "$DB_DIR/alivro.db" "$DB_DIR/alivro.sqlite" "/data/alivro.db" "/app/backend/alivro.db" 2>/dev/null || true
...
if bashio::config.has_value 'github_repo' && [ -n "$(bashio::config 'github_repo')" ]; then
	...
	export GITHUB_REPO="FaserF/Alivro"
...
download_file() {
	...
	local pub_code
	pub_code=$(curl -sSL --connect-timeout 15 --max-time 180 -w "%{http_code}" -H "Accept: application/vnd.github.v3+json" "$url" -o "$output" 2>/dev/null)
	...
	if [ -n "$token" ]; then
		...
		local http_code
		http_code=$(curl -sSL --connect-timeout 15 --max-time 180 -w "%{http_code}" -H "$auth_header" -H "Accept: application/vnd.github.v3+json" "$url" -o "$output" 2>/dev/null)
		...
		# Try raw tarball URL endpoint (Accept: application/vnd.github.raw)
		http_code=$(curl -sSL --connect-timeout 15 --max-time 180 -w "%{http_code}" -H "$auth_header" -H "Accept: application/vnd.github.raw" "$url" -o "$output" 2>/dev/null)
		if [ "$http_code" = "200" ] || [ "$http_code" = "302" ]; then
			if [ -s "$output" ]; then
		...
		# Try alternative: Direct GitHub archive URL (works for public / some setups)
		local direct_url
		...
		http_code=$(curl -sSL --connect-timeout 15 --max-time 180 -w "%{http_code}" -H "$auth_header" "$direct_url" -o "$output" 2>/dev/null)
		if [ "$http_code" = "200" ] || [ "$http_code" = "302" ]; then
			if [ -s "$output" ]; then
...
install_from_archive() {
	...
	bashio::log.info "Resolved Alivro Core Version: $CALCULATED_VERSION (Commit: ${DETECTED_SHA:-unknown})"
...
if [ ! -f "/app/backend/app/main.py" ] || [ ! -f "/app/frontend/index.html" ]; then
	...
	bashio::log.info "Attempting to download Alivro core..."
	...
	GITHUB_REPO_CONFIG="${GITHUB_REPO:-FaserF/Alivro}"
	...
	if [ "$DOWNLOAD_VERSION" = "latest" ]; then
		...
		pub_api_code=$(curl -s --connect-timeout 10 --max-time 20 -w "%{http_code}" -o /tmp/latest_release.json "https://api.github.com/repos/${GITHUB_REPO_CONFIG}/releases/latest" 2>/dev/null)
		if [ "$pub_api_code" = "200" ]; then
			...
			if [ -n "$GITHUB_TOKEN" ]; then
				...
				auth_api_code=$(curl -s --connect-timeout 10 --max-time 20 -w "%{http_code}" -H "$AUTH_HEADER" -o /tmp/latest_release.json "https://api.github.com/repos/${GITHUB_REPO_CONFIG}/releases/latest" 2>/dev/null)
	...
	# Download
	cd /tmp || exit 1
	if download_file "$DOWNLOAD_URL" "alivro.tar.gz" "$GITHUB_TOKEN"; then
		if install_from_archive "alivro.tar.gz" "/tmp/alivro-src"; then
			rm -f /tmp/alivro.tar.gz
...
if bashio::config.true 'developer_mode' && [ "${INITIAL_DOWNLOAD_DONE:-false}" != "true" ]; then
	...
	if download_file "$DOWNLOAD_URL" "main.tar.gz" "$GITHUB_TOKEN"; then
		if install_from_archive "main.tar.gz" "/tmp/alivro-main"; then
...
bashio::log.info "Creating .env file for backend..."
{
	...
	echo "DEMO_MODE_TYPE=${DEMO_MODE_TYPE:-}"
	[ -n "${TELEGRAM_BOT_TOKEN:-}" ] && echo "TELEGRAM_BOT_TOKEN=${TELEGRAM_BOT_TOKEN:-}"
	[ -n "${TELEGRAM_BOT_USERNAME:-}" ] && echo "TELEGRAM_BOT_USERNAME=${TELEGRAM_BOT_USERNAME:-}"
	...
	echo "ADDON_VERSION=${APP_VERSION:-${VERSION:-0.6.1}}"
	echo "APP_VERSION=${APP_VERSION:-${VERSION:-0.6.1}}"
	[ -n "${GIT_COMMIT:-}" ] && echo "GIT_COMMIT=${GIT_COMMIT}"
} >/app/backend/.env

# Run integration manager (install/update integration)
/usr/bin/alivro_integration.sh || true

# --- BACKEND START ---
bashio::log.info "Starting Alivro Backend (Uvicorn)..."
...
# Ensure uvicorn and zeroconf are installed in Python environment
if ! python3 -c "import uvicorn" 2>/dev/null; then
	bashio::log.warning "uvicorn module not found in Python environment. Auto-installing core dependencies..."
	python3 -m pip install --no-cache-dir "uvicorn[standard]>=0.23.0" "fastapi>=0.100.0" "pydantic-settings>=2.0.0" "sqlalchemy>=2.0.0" "aiosqlite>=0.19.0" "aiogram>=3.0.0" "python-dotenv>=1.0.0" "zeroconf>=0.130.0" || true
	if [ -f "/app/backend/requirements.txt" ]; then
		python3 -m pip install --no-cache-dir -r /app/backend/requirements.txt || true
	fi
elif ! python3 -c "import zeroconf" 2>/dev/null; then
	bashio::log.info "Installing zeroconf for Home Assistant auto-discovery..."
...
bashio::log.info "Alivro is now running!"
