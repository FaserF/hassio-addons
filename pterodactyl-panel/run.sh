...
_show_app_banner() {
    ...
    local GITHUB_PATH="pterodactyl-panel"
...
echo "This App requires the MariaDB core App 2.0 or newer!"
...
elif bashio::var.has_value "$(bashio::app.ingress_url)"; then
	APP_URL="$(bashio::app.ingress_url)"
...
# Disable SSL for database connection (MariaDB core App doesn't support SSL)
...
# Note: This is a "nice to have" feature - errors are logged but don't stop the App from starting
...
# This is optional - errors are logged but don't stop the App
...
EOF
	...
	# Run the script - errors are logged but don't stop the App
