...
5. In AdGuard Home "Client Settings", requests will appear as coming from the Addon's docker IP. To fix this, you may need to configure EDNS client subnet (ECS) if supported, but typically for Home Assistant Addons, source IP transparency is tricky without host networking.
