...
bashio::log.info "Starting ShieldDNS App..."
...
if bashio::fs.file_exists "${FULL_CERT_PATH}" && bashio::fs.file_exists "${FULL_KEY_PATH}"; then
	...
	openssl req -x509 -newkey rsa:4096 -sha256 -days 3650 \
		...
		-subj "/CN=shielddns-App" \
		-addext "subjectAltName=DNS:shielddns-App,IP:127.0.0.1"
...
if [ "${FALLBACK_DNS_ENABLED}" = "true" ]; then
	...
	# Using ping.
	# App environment usually has ping/nc. We'll use ping with timeout.
...
# The App info page is served by Nginx if enabled.
