...
# ==============================================================================
# ShieldDNS Integration Manager
# ==============================================================================
# This script manages the ShieldDNS integration in Home Assistant.
# It downloads the integration zip from the ha-shielddns releases.
# ==============================================================================
...
bashio::log.info "Checking ShieldDNS integration..."
...
local rel_code
rel_code=$(curl -s --connect-timeout 10 --max-time 30 -w "%{http_code}" -A "HomeAssistant-Addon" -o /tmp/shielddns_releases.json "https://api.github.com/repos/FaserF/ha-shielddns/releases" 2>/dev/null || echo "000")

if [ "$rel_code" = "200" ] && [ -s /tmp/shielddns_releases.json ]; then
	RELEASES_JSON=$(cat /tmp/shielddns_releases.json)
else
	check_github_status "$rel_code" "Releases check"
	RELEASES_JSON="[]"
fi
rm -f /tmp/shielddns_releases.json
...
if [ "$UPDATE_NEEDED" = "true" ]; then
	bashio::log.info "Installing/Updating ShieldDNS integration ($LOCAL_VERSION -> $TARGET_TAG)..."

	mkdir -p "/tmp/shielddns_install"
	rm -rf "/tmp/shielddns_install"
	mkdir -p "/tmp/shielddns_install"

	ZIP_URL="https://github.com/FaserF/ha-shielddns/releases/download/${TARGET_TAG}/shielddns.zip"
	local dl_code
	dl_code=$(curl -L -s --connect-timeout 10 --max-time 60 -w "%{http_code}" -o "/tmp/shielddns.zip" "$ZIP_URL" 2>/dev/null)
	if [ "$dl_code" != "200" ] || [ ! -s "/tmp/shielddns.zip" ]; then
		check_github_status "$dl_code" "Release package download"
		# Fallback to main branch zip if no target tag or error
		ZIP_URL="https://github.com/FaserF/ha-shielddns/archive/refs/heads/main.zip"
		bashio::log.info "Attempting main branch zip fallback: $ZIP_URL"
		curl -L -s --connect-timeout 10 --max-time 60 -o "/tmp/shielddns.zip" "$ZIP_URL" 2>/dev/null
	fi

	if [ -f "/tmp/shielddns.zip" ]; then
		if unzip -q "/tmp/shielddns.zip" -d "/tmp/shielddns_install"; then
			SRC_DIR=""
			if [ -f "/tmp/shielddns_install/manifest.json" ]; then
				SRC_DIR="/tmp/shielddns_install"
			elif [ -d "/tmp/shielddns_install/custom_components/shielddns" ]; then
				SRC_DIR="/tmp/shielddns_install/custom_components/shielddns"
			else
				nested_dir=$(find /tmp/shielddns_install -name "manifest.json" -exec dirname {} \; | head -n 1)
			...
			if [ -n "$SRC_DIR" ] && [ -f "$SRC_DIR/manifest.json" ]; then
				...
				bashio::log.green "ShieldDNS integration successfully updated to $TARGET_TAG."
		...
		rm -f "/tmp/shielddns.zip"
	else
		bashio::log.error "❌ Download failed."
	fi
	rm -rf "/tmp/shielddns_install"
else
	bashio::log.info "ShieldDNS integration is up to date ($LOCAL_VERSION)."
fi
...
DISCOVERY_RESPONSE=$(curl -s -w "\nHTTP_STATUS:%{http_code}" -X POST \
	...
	-d "{\"service\":\"shielddns\",\"config\":{\"addon\":\"$SLUG\"}}" \
