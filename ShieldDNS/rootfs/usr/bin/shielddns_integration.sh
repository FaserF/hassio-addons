...
RELEASES_JSON=$(curl -s --connect-timeout 10 --max-time 30 -A "HomeAssistant-Addon" "https://api.github.com/repos/FaserF/ha-shielddns/releases")
...
if [ "$UPDATE_NEEDED" = "true" ]; then
	...
	if ! curl -L -s -f -o "/tmp/shielddns.zip" "$ZIP_URL"; then
		...
		if [ -z "$TARGET_TAG" ]; then
			...
			curl -L -s -f -o "/tmp/shielddns.zip" "$ZIP_URL"
		fi
	fi
