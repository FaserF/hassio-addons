#!/usr/bin/env python3
"""
Inject app startup banner into Dockerfiles and run scripts during CI build.
...
Usage:
    python3 inject_version_warning.py <app_path>
"""
...
def inject_dockerfile_env(addon_path: str, addon_info: dict, unsupported: bool) -> bool:
    ...
    if not os.path.exists(dockerfile_path):
        print(f"⚠️ No Dockerfile found in {addon_path}")
        return False
...
def main():
    ...
    if not os.path.isdir(addon_path):
        print(f"❌ Addon path not found: {addon_path}")
        sys.exit(1)

    print(f"🔧 Processing addon: {addon_path}")
