module.exports = async ({ github, context, core }) => {
  ...
  const infraPatterns = [
    {
      pattern: /action.*not found/i,
      type: 'Action Not Found',
      fix: 'Update action to an existing version',
    },
    {
      pattern: /version.*deprecated/i,
      type: 'Deprecated Version',
      fix: 'Update to a supported version',
    },
    {
      pattern: /runner.*unavailable/i,
      type: 'Runner Unavailable',
      fix: 'Check runner labels and availability',
    },
    ...
    {
      pattern: /yaml.*error|syntax error/i,
      type: 'YAML/Syntax Error',
      fix: 'Run actionlint locally to fix syntax',
    },
    { pattern: /permission denied/i, type: 'Permission Error', fix: 'Check workflow permissions' },
    {
      pattern: /node.*version|unsupported.*node/i,
      type: 'Node Version Issue',
      fix: 'Update to Node.js 20',
    },
    {
      pattern: /docker.*error|container.*error/i,
      type: 'Docker Error',
    ...
    },
    {
      ...
      fix: 'Investigate concurrent run conflicts',
    },
    {
      pattern: /unauthorized|authentication failed/i,
      type: 'Auth Failure',
      fix: 'Check secrets and permissions',
    },
    {
      pattern: /network.*error|connection.*refused/i,
      type: 'Network Error',
      fix: 'Retry or check external service status',
    },
    {
      pattern: /disk.*space|no space left/i,
      type: 'Disk Space',
      fix: 'Clean up runner or optimize build artifacts',
    },
    {
      pattern: /cache.*error|restore.*fail/i,
      type: 'Cache Error',
      fix: 'Clear cache or check keys',
    },
  ];
  ...
  for (const job of failedJobs) {
    ...
    const stepName =
      failedStep?.name || (job.status === 'completed' ? 'Setup/Infrastructure' : 'Unknown');
  ...
  if (infraErrors.length === 0) {
    console.log(
      'No infrastructure errors detected. Project test failures are handled by CI workflow.'
    );
    return;
  }
