...
bashio::log.info "Starting Solumati App initialization..."
...
if bashio::config.true 'factory_reset'; then
	...
	bashio::log.info ""
	bashio::log.warning "IMPORTANT: Disable 'factory_reset' in the App settings!"
...
if bashio::config.has_value 'app_base_url' && [ -n "$(bashio::config 'app_base_url')" ]; then
	...
	if bashio::var.has_value "$(bashio::app.ingress_url)"; then
		APP_BASE_URL="$(bashio::app.ingress_url)"
		export APP_BASE_URL
...
if bashio::config.true 'dev_use_main_branch'; then
	...
	if download_branch "main" || download_branch "master"; then
		...
		bashio::log.error "If this is a private repository, please configure 'github_token' in the App options."
	fi
else
