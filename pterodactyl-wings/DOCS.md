...
1. **Start the app**: If the configuration file defined in `config_file` does not exist, the app will create a default one for you at that location.
2. **Access Pterodactyl Panel**: Go to your Pterodactyl Panel (e.g., using the `pterodactyl-panel` app).
...
6. **Restart Wings**: Restart this app to apply the configuration.

## 📂 Folder Usage

- `/share`: Mapped for sharing files between Wings and the Panel or other apps.
