...
if [ -z "$username" ] || [ "$username" = "null" ] || [ -z "$password" ] || [ "$password" = "null" ]; then
	...
	bashio::log.error "=========================================="
	bashio::log.error "Please configure your Tado username and password in the App configuration."
	bashio::log.error ""
	...
	bashio::log.error ""
	bashio::log.error "You can find these settings in the 'Configuration' tab of this App."
	bashio::exit.nok
fi
...
if [ -f /data/refresh_token ]; then
	...
	bashio::log.warning "and restarting the App to force a fresh login."
fi
