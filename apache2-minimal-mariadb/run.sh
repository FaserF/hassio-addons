...
_show_app_banner() {
	...
	local GITHUB_PATH="apache2-minimal-mariadb"
...
external_port=""
if [ -n "$SUPERVISOR_TOKEN" ]; then
	App_info=$(curl -sSL -H "Authorization: Bearer $SUPERVISOR_TOKEN" http://supervisor/Apps/self/info 2>/dev/null)
	if [ -n "$App_info" ] && echo "$App_info" | jq -e . >/dev/null 2>&1; then
		external_port=$(echo "$App_info" | jq -r ".data.network.\"${port}/tcp\"?" 2>/dev/null)
	fi
fi
